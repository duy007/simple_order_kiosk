...
CREATE TABLE SALE(sale_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, cart_id INTEGER NOT NULL UNIQUE, sale_date DATE DEFAULT CURRENT_TIMESTAMP NOT NULL, sale_customer_num INTEGER DEFAULT (ABS(RANDOM() % 500) + 1) NOT NULL, payment_method TEXT NOT NULL, total_price INTEGER NOT NULL, tax_percentage REAL DEFAULT 0.12 NOT NULL, tax INTEGER NOT NULL, tip_percentage REAL NOT NULL, total_tip INTEGER NOT NULL, total_amount INTEGER NOT NULL, sale_completed boolean NOT NULL, FOREIGN KEY(cart_id) REFERENCES Cart(cart_id) ON DELETE CASCADE ON UPDATE NO ACTION);

CREATE TABLE orderItem(order_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, cart_id INTEGER NOT NULL, product_id INTEGER NOT NULL, sugar_level REAL NOT NULL, ice_level TEXT NOT NULL, quantity INTEGER DEFAULT 1 NOT NULL, FOREIGN KEY(cart_id) REFERENCES Cart(cart_id) ON DELETE CASCADE ON UPDATE NO ACTION, FOREIGN KEY(product_id) REFERENCES Product(product_id) ON DELETE CASCADE ON UPDATE NO ACTION);

CREATE TABLE orderTopping(
order_topping_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, order_id INTEGER NOT NULL, topping_id INTEGER NOT NULL, extra BooLEAN DEFAULT FALSE NOT NULL, FOREIGN KEY(order_id) REFERENCES orderItem(order_id) ON DELETE CASCADE ON UPDATE NO ACTION, FOREIGN KEY(topping_id) REFERENCES Topping(topping_id) ON DELETE CASCADE ON UPDATE NO ACTION);"
