...
CREATE TABLE Topping (topping_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, topping_name TEXT NOT NULL UNIQUE, price NUMERIC NOT NULL, extra_price NUMERIC NOT NULL);
...
CREATE TABLE SALE(sale_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, cart_id INTEGER NOT NULL UNIQUE, sale_date DATE DEFAULT CURRENT_TIMESTAMP NOT NULL, sale_customer_num INTEGER DEFAULT (ABS(RANDOM() % 500) + 1) NOT NULL, payment_method TEXT NOT NULL, total_price NUMERIC NOT NULL, tax_percentage NUMERIC DEFAULT 0.12 NOT NULL, tax NUMERIC NOT NULL, tip_percentage NUMERIC NOT NULL, total_tip NUMERIC NOT NULL, total_amount NUMERIC NOT NULL, sale_completed boolean NOT NULL, FOREIGN KEY(cart_id) REFERENCES Cart(cart_id) ON DELETE CASCADE ON UPDATE NO ACTION);
